...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8 select-none">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-primary)] tracking-wide">{t.dashboard.title}</h1>
        <p className="text-sm text-zinc-500 mt-1">{t.dashboard.subtitle}</p>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {/* Projects Section */}
        <div className="bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl p-6 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-2.5">
                <FolderGit2 className="h-5 w-5 text-indigo-500" />
                <h2 className="text-base font-bold text-[var(--text-primary)]">{t.dashboard.projects_title}</h2>
              </div>
              <button 
                onClick={() => navigate('/projects')}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-xs font-semibold text-[var(--text-primary)] transition-colors"
              >
            ...
            {projects.length === 0 ? (
              ...
              <div className="space-y-3">
                {projects.map((project) => (
                  <div
                    key={project.id}
                    className="p-4 rounded-lg bg-[var(--bg-tertiary)]/55 border border-[var(--border-color)]/35 flex items-center justify-between hover:border-zinc-500/50 transition-colors"
                  >
                    <div>
          ...
          <div>
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="text-base font-bold text-[var(--text-primary)]">{t.dashboard.servers_title}</h2>
              </div>
              <button 
              ...
              {mockServers.map((server) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-sm font-bold text-[var(--text-primary)]">{server.name}</span>
